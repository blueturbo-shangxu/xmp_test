...
class OAuthToken(Base):
    ...
    email = Column(String(255), index=True, comment='用户邮箱地址')
    ...
    advertiser_ids = Column(JSONB, default=[], comment='成功同步的广告账户ID列表')
...
class SyncTask(Base):
    ...
    error_traceback = Column(Text, comment='错误堆栈详情')
    ...
    # 同步参数
    sync_params = Column(JSONB, default={}, comment='同步参数条件(如: {"ad_group_id": "123", "campaign_id": "456"})')

    # 发起者信息
    initiator_type = Column(String(20), default='PROGRAM', comment='发起者类型: USER, PROGRAM')
    initiator_id = Column(String(100), comment='发起者ID: 用户ID或程序代号')

    # 重试机制
    retry_count = Column(Integer, default=0, comment='当前重试次数')
    max_retry_count = Column(Integer, default=3, comment='最大重试次数')
    retry_interval_seconds = Column(Integer, default=60, comment='重试间隔(秒)')
    ...
    __table_args__ = (
        ...
        CheckConstraint("initiator_type IN ('USER', 'PROGRAM')", name='ck_initiator_type'),
    )
...
class GoogleAdGroup(Base):
    ...
    # 关系 - ads
    ads = relationship("GoogleAd", back_populates="ad_group", cascade="all, delete-orphan")
...
class GoogleAd(Base):
    ...
    account_id = Column(BigInteger, ForeignKey('google_ad_accounts.id', ondelete='CASCADE'), nullable=False, index=True)
    campaign_id = Column(BigInteger, ForeignKey('google_campaigns.id', ondelete='CASCADE'), nullable=False, index=True)
    ad_group_id = Column(BigInteger, ForeignKey('google_ad_groups.id', ondelete='CASCADE'), nullable=False, index=True)
    customer_id = Column(String(50), nullable=False, index=True, comment='关联的Google Ads Customer ID')
    ad_id = Column(String(50), nullable=False, comment='Google Ads Ad ID')
    ad_name = Column(String(255), comment='广告名称')
    ad_status = Column(String(20), default='ENABLED', index=True, comment='状态')
    ad_type = Column(String(50), comment='广告类型')
    # 响应式搜索广告字段
    headlines = Column(JSONB, comment='标题列表')
    descriptions = Column(JSONB, comment='描述列表')
    # 展示广告字段
    final_urls = Column(JSONB, comment='最终到达网址列表')
    final_mobile_urls = Column(JSONB, comment='移动端最终到达网址列表')
    tracking_url_template = Column(Text, comment='跟踪网址模板')
    # 其他字段
    display_url = Column(String(500), comment='显示网址')
    policy_summary = Column(JSONB, comment='政策摘要')
    raw_data = Column(JSONB, comment='原始数据(JSON)')
    ext_info = Column(JSONB, default={}, comment='扩展信息(预留字段)')
    last_synced_at = Column(TIMESTAMP, index=True, comment='最后同步时间')
    created_at = Column(TIMESTAMP, server_default=func.now())
    ...
    # 关系
    account = relationship("GoogleAdAccount", backref="ads")
    campaign = relationship("GoogleCampaign", backref="ads")
    ad_group = relationship("GoogleAdGroup", back_populates="ads")

    __table_args__ = (
        Index('uk_google_customer_ad', 'customer_id', 'ad_id', unique=True),
        CheckConstraint("ad_status IN ('ENABLED', 'PAUSED', 'REMOVED')", name='ck_google_ad_status'),
    )

    def __repr__(self):
        return f"<GoogleAd(ad_id='{self.ad_id}', type='{self.ad_type}')>"
